...
void Warrior::setMovingIntRect(int counter, bool isEnemy) {
    if (isEnemy) {
        m_sprite.setTextureRect(
                sf::IntRect(MOVING_ANIMATION_ARR[counter], IMAGE_HEIGHT * 3, IMAGE_WIDTH, IMAGE_HEIGHT));
    } else {
...
void Warrior::setWeapon(Weapons_t weapon, bool visible) {
    ...
    new_weapon->setSpriteLoc(sf::Vector2f(m_sprite.getPosition().x + m_sprite.getGlobalBounds().width * 0.25, m_sprite.getPosition().y + m_sprite.getGlobalBounds().height * 0.125));
